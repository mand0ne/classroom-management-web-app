...
window.onclick = event => {
    ...
    if (!isFinite(kliknutiDan) || kliknutiDan < 1 || kliknutiDan > 31 || ![0, 1, 2, 3, 4, 5, 9, 10, 11].includes(Kalendar.trenutniMjesec()) || !trenutniPocetak.length > 0 || !trenutniKraj.length > 0 || document.getElementById("err").innerHTML.length > 0)
        return;
    ...
    if (izabraniDatum.getTime() <= trenutniDatum.getTime()) {
        alert("Nema smisla izabrat datum koji je već prošao...");
        return 0;
    }
    ...
    var datumZauzeca = new Date(new Date().getFullYear(), Kalendar.trenutniMjesec(), kliknutiDan);
    ...
    if (periodicno)
        ...
        poruka = "Rezervacija sale " + trenutnaSala + " na datum " + datumZauzeca.toLocaleDateString("en-UK") + " u periodu od " + trenutniPocetak + " do " + trenutniKraj + ".\nPritisnite 'OK' za potvrdu";
